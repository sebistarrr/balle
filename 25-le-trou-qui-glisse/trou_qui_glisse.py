"""
Le trou qui glisse — rendu vidéo, format vertical.

La boîte percée (23), mais la brèche ne reste plus en place : elle glisse
le long du plancher, d'un coin à l'autre et retour, en sept secondes. Deux
balles au départ, et deux de plus à chaque sortie.

Ce seul changement retourne la partie. Une brèche fixe finit par se boucher :
les balles s'entassent devant, celles du dessus ne la voient jamais, et le débit
tombe. Une brèche mobile va chercher les balles là où elles sont, et le tas se
vide par la tranche qu'elle traverse.

Rendu :
    python trou_qui_glisse.py                 # écrit trou_qui_glisse.mp4 en 1080 x 1920, 60 im/s
    python trou_qui_glisse.py --graines       # essaie des tirages et donne ce qu'ils valent
...
#  La brèche ne reste pas en place : elle glisse le long du bord du bas, d'un
#  coin à l'autre et retour. Fixe, elle se bouche — les balles s'entassent
#  devant, celles du dessus ne la voient jamais, et le débit tombe. Mobile, elle
#  vient chercher les balles là où elles sont.
TROU_L = 330.0                    # largeur de la brèche
TROU_PER = 7.0                    # s pour un aller-retour complet


def trou_x(t):
    return ((X0 + X1) / 2
            + (X1 - X0 - TROU_L - 40) / 2 * math.sin(TAU * t / TROU_PER))
R_BALLE = 11.0
...
class Partie:
    ...
    def pas(self):
        ...
        tg = trou_x(self.t) - TROU_L / 2
        td = trou_x(self.t) + TROU_L / 2
...
def dessiner(ctx, jeu, dt):
    ...
    tg, td = trou_x(jeu.t) - TROU_L / 2, trou_x(jeu.t) + TROU_L / 2
    ctx.move_to(X0, Y1); ctx.line_to(tg, Y1)
    ctx.move_to(td, Y1); ctx.line_to(X1, Y1)
    ctx.stroke()
    #  Deux repères verts aux lèvres de la brèche : sur un bord qui glisse, il
    #  faut voir où elle est avant qu'une balle n'y tombe.
    ctx.set_source_rgb(0.20, 1.0, 0.55)
    ctx.set_line_width(5)
    ctx.new_path()
    ctx.move_to(tg, Y1 - 26); ctx.line_to(tg, Y1 + 26)
    ctx.move_to(td, Y1 - 26); ctx.line_to(td, Y1 + 26)
    ctx.stroke()

    troupeau(ctx, jeu.dx, jeu.dy, jeu.dh)
    troupeau(ctx, jeu.x, jeu.y, jeu.h)

    texte(ctx, "+2 balles, et le trou qui glisse", W / 2, 96, 44, (0.92, 0.96, 0.97))
...
def rendre(graine=GRAINE, sortie="trou_qui_glisse.mp4"):
    jeu = Partie(graine)
    surf = cairo.ImageSurface(cairo.FORMAT_ARGB32, W, H)
    ctx = cairo.Context(surf)

    muet = "trou_qui_glisse-muet.mp4"
    ...
    duree = images / FPS
    son = bande_son(jeu.notes, jeu.fini or duree, duree, "trou_qui_glisse.wav")
